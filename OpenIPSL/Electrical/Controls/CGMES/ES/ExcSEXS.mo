...
model ExcSEXS "Simplified Excitation System Model from CGMES definition"
  ...
  parameter Real T_AT_B "Ta/Tb - gain reduction ratio of lag-lead element (TaTb).  Typical Value = 0.1";
  parameter Real T_B "Denominator time constant of lag-lead block (Tb).  Typical Value = 10";
  ...
  parameter Real T_C "PI controller phase lead time constant (Tc).  Typical Value = 0";
  parameter Real E_MIN "Minimum field voltage output (Emin).  Typical Value = -5";
  parameter Real E_MAX "Maximum field voltage output (Emax).  Typical Value = 5";
  parameter Real EFD_MAX "Field voltage clipping maximum limit (Efdmax).  Typical Value = 5";
  parameter Real EFD_MIN "Field voltage clipping minimum limit (Efdmin).  Typical Value = -5";
  ...
  Modelica.Blocks.Nonlinear.Limiter limiter(uMax=EFD_MAX, uMin=EFD_MIN) annotation (Placement(transformation(extent={{160,-10},{180,10}})));
...
equation
  ...
  connect(leadLag.u, V_erro.y) annotation (Line(points={{8,0},{0,0},{-19,0}}, color={0,0,127}));
  connect(simpleLagLim.y, limiter.u) annotation (Line(points={{141,0},{149.5,0},{158,0}}, color={0,0,127}));
  connect(limiter.y, EFD) annotation (Line(points={{181,0},{210,0}}, color={0,0,127}));
  connect(leadLag.y, transferFunction.u) annotation (Line(points={{31,0},{56,0}}, color={0,0,127}));
  connect(transferFunction.y, simpleLagLim.u) annotation (Line(points={{79,0},{98.5,0},{118,0}}, color={0,0,127}));
  annotation (
